...
export default (store, options) => {
  ...
  const coll = store.db.collection(`${name}.events`);
  ...
  const get = (id, snap, fn) => {
    ...
    const query = snap ? { id: snap.id, scope: snap.scope, revision: { $gt: snap.revision } } : { id, scope };
  ...
  const getMulti = (ids, snaps, fn) => coll.find({
    $or: ids.map(id => {
      const snap = snaps.length ? snaps.find(s => s.id === id && s.scope === scope) : null;
      return snap ? { id, scope: snap.scope, version: { $gt: snap.revision } } : { id, scope };
    })
