...
export default (store, options) => {
  ...
  const { name, scope } = store;
  const { rounds = 10 } = options;
  const coll = store.db.collection(`${name}.snapshots`);

  coll.ensureIndex({ id: 1, revision: 1 }, error);
  coll.ensureIndex({ version: 1 }, error);
  coll.ensureIndex({ scope: 1 }, error);
  ...
  const get = (id, fn) => {
    ...
    coll.find({ id, scope }).limit(-1).sort({ version: -1 }).next(fn);
  }
  ...
  const getMulti = (ids, fn) => coll.aggregate(...[
    { $match: { id: { $in: ids }, scope } },
    { $group: { _id: '$id', revision: { $last: '$revision' } } },
    { $project: { _id: 0, id: '$_id.id', revision: 1, scope: 1 }}
  ], (err, pairs) => {
  ...
  const commit = (entity, options = {}, fn = noop) => {
    ...
    if (options.snap || rounds && entity.revision >= entity.version + rounds) {
  ...
  const commitMulti = (entities, options = {}, fn) => {
    const snaps = entities
      .filter(e => options.snap || rounds && e.revision >= e.version + rounds)
